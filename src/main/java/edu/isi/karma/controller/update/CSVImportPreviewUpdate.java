...
public class CSVImportPreviewUpdate extends AbstractUpdate {
	...
	private static Logger logger = LoggerFactory.getLogger(CSVImportPreviewUpdate.class.getSimpleName());
	
	public enum JsonKeys {
		commandId, headers, rows
	}
	...
	public void generateJson(String prefix, PrintWriter pw,
		...
		try {
			...
			JSONWriter writer = jsonStr.object().key("commandId").value(commandId)
				.key("updateType").value("ImportCSVPreview").key("fileName").value(csvFile.getName());
			...
			while (scanner.hasNextLine()) {
	        	...
	        	if(rowCount+1 >= dataStartRowIndex && rowCount+1 < dataStartRowIndex+5) {

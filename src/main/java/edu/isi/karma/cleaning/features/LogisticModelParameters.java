...
public class LogisticModelParameters implements Writable {
	...
	/**
	 * Returns a CsvRecordFactory compatible with this logistic model. The
	 * reason that this is tied in here is so that we have access to the list of
	 * target categories when it comes time to save the model. If the input
	 * isn't CSV, then calling setTargetCategories before calling saveTo will
	 * suffice.
	 * 
	...
	public CsvRecordFactory getCsvRecordFactory() {
		if (csv == null) {
			csv = new CsvRecordFactory(getTargetVariable(), getTypeMap())
					.maxTargetValue(getMaxTargetCategories()).includeBiasTerm(
							useBias());
	...
	/**
	 * Creates a logistic regression trainer using the parameters collected
	 * here.
	 * 
	...
	public OnlineLogisticRegression createRegression() {
		if (lr == null) {
			lr = new OnlineLogisticRegression(getMaxTargetCategories(),
					getNumFeatures(), new L1()).lambda(getLambda())
					.learningRate(getLearningRate()).alpha(1 - 1.0e-3);
		}
		return lr;
	}

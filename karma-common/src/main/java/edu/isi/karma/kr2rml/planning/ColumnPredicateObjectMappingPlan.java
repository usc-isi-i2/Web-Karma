...
public class ColumnPredicateObjectMappingPlan extends
	...
	protected void outputTriple(KR2RMLRDFWriter outWriter,
		...
		{
			outWriter.outputTripleWithURIObject(pom.getTriplesMap().getId(), subject.getURI(), predicate.getURI(), uriFormatter.getExpandedAndNormalizedUri(object.getURI()));
		}
		else if(generateContext && objectTemplateTermSetPopulator.getTerms().isSingleColumnTerm())
		{
			try {
				outWriter.outputQuadWithLiteralObject(pom.getTriplesMap().getId(), subject.getURI(), predicate.getURI(), object.getURI(), literalTemplateValue,getColumnContextUri(translator.getHNodeIdForColumnName(objectTemplateTermSetPopulator.getTerms().getAllTerms().get(0).getTemplateTermValue())));
		...
		{
			outWriter.outputTripleWithLiteralObject(pom.getTriplesMap().getId(), subject.getURI(), predicate.getURI(), object.getURI(), literalTemplateValue);
		}

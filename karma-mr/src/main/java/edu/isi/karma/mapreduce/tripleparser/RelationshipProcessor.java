...
public class RelationshipProcessor implements Runnable {

	private final static Logger logger = LoggerFactory.getLogger(RelationshipProcessor.class);
	Configuration conf;
	Path input; 
	Path output;
	
	public RelationshipProcessor(Configuration conf, Path input, Path output) {
		this.conf = conf;
		this.input = input;
		this.output = output;
	}

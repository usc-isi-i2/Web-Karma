...
public class Prober {
	...
	public static String PartitionDisplay1(Vector<Partition> vp) {
		...
		for (Partition p : vp) {
			...
			for (int i = 0; i < p.tarNodes.size(); i++) {
				String line = UtilTools.print(p.orgNodes.get(i)) + "|" + UtilTools.print(p.tarNodes.get(i));
				res += line + "\n";
			}
		}
		return res;
	...
	public static void printRecommendation(DataRecord record, ArrayList<Double> alllabels) {
		System.out.println(record.origin + ", " + record.transformed);

        ...
        init: function()
        {
			...
			var dealWithNode = function(curNode, clickNode, parentInvisble) {
				...
				if(hideable) {
					...
					var parVis = isRoot ? true : curNode.parent().parent().data("visible");
					var parGra = isRoot ? false : curNode.parent().parent().children().children().children().first().css("background-color") == "rgb(128, 128, 128)";
					if(clickNode)	curNode.data("visible", !curNode.data("visible"));
					var styleNode = curNode.children().children().children().first();
            ...
            var data,
                ...
                step  = function(level, depth)
                {
                    ...
                    items.each(function()
                    {
                        var li   = $(this),
                            //item = $.extend({}, li.data()),
                        	item = $.extend({}, {"id": li.data("id"), "name": li.data("name"), "visible": li.data("visible"), "hideable":  li.data("hideable")});
...
//            console.log("Set scrollTop: " + newScroll + "=>" + mouse.lastY + ":" + mouse.nowY + ", "  + e.pageY + ":" + mouse.offsetY);
            ...
            if (!mouse.dirAx || isNewRoot || isEmpty) {
				...
				if(this.pointEl.parent()[0] == this.placeEl.parent()[0]) {
                if (isEmpty) {
